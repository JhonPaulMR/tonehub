...
- [ ] Criar `AuthController` com métodos: `showLogin`, `login`, `showRegister`, `register`, `logout`
- [ ] Implementar registro com `Hash::make()` e `Auth::login()` automático
- [ ] Implementar login com `Auth::attempt()` e suporte a `remember`
- [ ] Implementar logout com `Auth::logout()` e invalidação de sessão
- [ ] Definir rotas de autenticação: `GET/POST /login`, `GET/POST /register`, `POST /logout`
- [ ] Criar `ItemPolicy` com métodos `update()` e `delete()` (verificação `$user->id === $item->user_id`)
- [ ] Registrar a Policy (auto-discovery ou `AuthServiceProvider`)
- [ ] Aplicar `$this->authorize()` nos controllers `edit`, `update`, `destroy`
...
- [ ] Executar `php artisan storage:link` para criar symlink público
- [ ] Criar `StoreItemRequest` com regras de validação:
...
- [ ] Criar `UpdateItemRequest` (mesmas regras, `preset_file` nullable)
- [ ] Implementar upload no `ItemController::store()` usando `Storage::disk('public')`
  - Subpastas organizadas: `covers/`, `presets/`, `samples/`
- [ ] Implementar lógica de download com incremento de `downloads_count`
- [ ] Implementar exclusão de arquivos antigos no `update()` e `destroy()`
- [ ] Implementar lógica de sincronização de tags (parse de string separada por vírgula)
...
- [ ] Implementar `scopeSearch()` no Model `Item`:
  - Busca por `title`, `hardware_model`, `description` e `tags.name`
  - Usar `orWhereHas('tags', ...)` para busca em relações N:M
- [ ] Integrar busca no `HomeController` com `$request->input('search')`
- [ ] Criar `ItemFactory` com dados fake realistas (nomes de amps, categorias aleatórias)
- [ ] Criar `TagFactory`
...
- [ ] Criar layout base `resources/views/layouts/app.blade.php`:
  - Navbar com logo, barra de busca, links de autenticação (`@auth`/`@guest`)
  - Footer com créditos
  - Uso de `@yield('content')` e `@stack('scripts')`
- [ ] Criar componente `<x-item-card>`: card do preset com capa, título, categoria, tags, downloads
- [ ] Criar componente `<x-audio-player>`: toggle Dry/Wet com Alpine.js e `<audio>` nativo
- [ ] Criar componente `<x-tag-badge>`: badge estilizado para tags
- [ ] Criar componente `<x-flash-message>`: mensagens de sucesso/erro com animação

### Páginas

- [ ] `home.blade.php`: Feed principal com grid de cards, barra de busca, paginação
- [ ] `items/show.blade.php`: Detalhe do item com player, info completa, botão de download
- [ ] `items/create.blade.php`: Formulário de upload com preview de imagem
- [ ] `items/edit.blade.php`: Formulário de edição (reuso de partials)
- [ ] `dashboard/index.blade.php`: Tabela de gerenciamento dos items do usuário
- [ ] `auth/login.blade.php`: Formulário de login estilizado
- [ ] `auth/register.blade.php`: Formulário de registro estilizado

### Estilização TailwindCSS (Dark Mode)

- [ ] Tema escuro como padrão: fundo `gray-900`/`gray-800`
- [ ] Acentos em `amber-500`/`orange-500` (identidade visual de guitarra)
- [ ] Cards com `rounded-xl shadow-lg hover:shadow-amber-500/20 transition`
- [ ] Botões com gradiente `from-amber-500 to-orange-600`
- [ ] Responsividade completa: `grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4`
- [ ] Formulários: inputs com `bg-gray-700 border-gray-600 focus:ring-amber-500`

### Finalização

- [ ] Popular banco com `php artisan migrate:fresh --seed` (50 items para demo)
- [ ] Atualizar `README.md` com print da tela principal
- [ ] Criar `.env.example` limpo
- [ ] Testar fluxo completo: registro → login → upload → busca → download → editar → deletar
- [ ] Testar que Policy bloqueia edição/exclusão por não-donos
